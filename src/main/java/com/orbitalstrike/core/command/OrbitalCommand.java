...
public class OrbitalCommand {
            ...
            dispatcher.register(
                    CommandManager.literal("orbital")
                            ...
                            .then(CommandManager.literal("setting")
                                    ...
                                    // accuratestab branch
                                    .then(CommandManager.literal("accuratestab")
                                            .then(CommandManager.literal("PIERCER_COUNT")
                                                    .then(CommandManager.argument("value", IntegerArgumentType.integer(1))
                                                            .executes(ctx -> {
                                                                AccurateStabShot.PIERCER_COUNT = IntegerArgumentType.getInteger(ctx, "value");
                                                                ctx.getSource().sendFeedback(() -> Text.literal("AccurateStab piercer count set to " + AccurateStabShot.PIERCER_COUNT), false);
                                                                return 1;
                                                            })
                                                    )
                                            )
                                            .then(CommandManager.literal("ACCELERATOR_COUNT")
                                                    .then(CommandManager.argument("value", IntegerArgumentType.integer(1))
                                                            .executes(ctx -> {
                                                                AccurateStabShot.ACCELERATOR_COUNT = IntegerArgumentType.getInteger(ctx, "value");
                                                                ctx.getSource().sendFeedback(() -> Text.literal("AccurateStab accelerator count set to " + AccurateStabShot.ACCELERATOR_COUNT), false);
                                                                return 1;
                                                            })
                                                    )
                                            )
                                            .then(CommandManager.literal("Y_HEIGHT")
                                                    .then(CommandManager.argument("value", IntegerArgumentType.integer(1))
                                                            .executes(ctx -> {
                                                                AccurateStabShot.Y_HEIGHT = IntegerArgumentType.getInteger(ctx, "value");
                                                                ctx.getSource().sendFeedback(() -> Text.literal("AccurateStab Y height set to " + AccurateStabShot.Y_HEIGHT), false);
                                                                return 1;
                                                            })
                                                    )
                                            )
                                            .then(CommandManager.literal("ACCELERATOR_FUSE")
                                                    .then(CommandManager.argument("value", IntegerArgumentType.integer(1))
                                                            .executes(ctx -> {
                                                                AccurateStabShot.ACCELERATOR_FUSE = IntegerArgumentType.getInteger(ctx, "value");
                                                                ctx.getSource().sendFeedback(() -> Text.literal("AccurateStab accelerator fuse set to " + AccurateStabShot.ACCELERATOR_FUSE), false);
                                                                return 1;
                                                            })
                                                    )
                                            )
                                            .then(CommandManager.literal("PIERCER_FUSE")
                                                    .then(CommandManager.argument("value", IntegerArgumentType.integer(1))
                                                            .executes(ctx -> {
                                                                AccurateStabShot.PERICER_FUSE = IntegerArgumentType.getInteger(ctx, "value");
                                                                ctx.getSource().sendFeedback(() -> Text.literal("AccurateStab piercer fuse set to " + AccurateStabShot.PERICER_FUSE), false);
                                                                return 1;
                                                            })
                                                    )
                                            )
    ...
    private static int strikePos(CommandContext<ServerCommandSource> ctx) {
        ...
        int finalSize = size;
        StrikeScheduler.schedule(delay, () -> shot.fire(ctx.getSource().getWorld(), pos.toCenterPos(), finalSize));
    ...
    private static int strikePosNoSize(CommandContext<ServerCommandSource> ctx) {
        ...
        StrikeScheduler.schedule(delay, () -> shot.fire(ctx.getSource().getWorld(), pos.toCenterPos(), 0));
    ...
    private static int strikeCrosshair(CommandContext<ServerCommandSource> ctx) {
        ...
        int finalSize = size;
        StrikeScheduler.schedule(delay, () -> shot.fire(ctx.getSource().getWorld(), hit.getBlockPos().toCenterPos(), finalSize));
    ...
    private static int strikeCrosshairNoSize(CommandContext<ServerCommandSource> ctx) {
        ...
        StrikeScheduler.schedule(delay, () -> shot.fire(ctx.getSource().getWorld(), hit.getBlockPos().toCenterPos(), 0));
    ...
    private static int giveFixedStrikeRod(CommandContext<ServerCommandSource> ctx) {
        ...
        Text rodName;
        if (ADVANCED_ROD_NAMES) {
            rodName = Text.literal(shotId + " " + delay + " " + size + " " + pos.toShortString()).styled(style -> style.withItalic(true));
        } else {
            rodName = Text.literal(displayName + " shot").styled(style -> style.withItalic(true));
        }
        if (FANCY_RODS) {
    ...
    private static int giveFixedStrikeRodNoSize(CommandContext<ServerCommandSource> ctx) {
        ...
        Text rodName;
        if (ADVANCED_ROD_NAMES) {
            rodName = Text.literal(shotId + " " + delay + " 0 " + pos.toShortString()).styled(style -> style.withItalic(true));
        } else {
            rodName = Text.literal(displayName + " shot").styled(style -> style.withItalic(true));
        }
        if (FANCY_RODS) {
    ...
    private static int help(CommandContext<ServerCommandSource> ctx) { return 1; }
    private static int helpSetting(CommandContext<ServerCommandSource> ctx) { return 1; }
    private static int craft(CommandContext<ServerCommandSource> ctx) { return 1; }
    private static int auth(CommandContext<ServerCommandSource> ctx) { return 1; }
}