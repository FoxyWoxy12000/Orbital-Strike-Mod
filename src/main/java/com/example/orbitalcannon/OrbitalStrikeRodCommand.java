...
public class OrbitalStrikeRodCommand {

    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(
                Commands.literal("orbitalstrikerod")
                        .requires(src -> src.hasPermission(2))
                        .then(Commands.argument("type", StringArgumentType.word())
                                .suggests((ctx, builder) -> OrbitalStrikeRodCommandSuggestions.suggestTypes(ctx, builder))
    ...
    private static int execute(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        // Wemmbu check uses server-level gamerule - consistent with other commands
        boolean wemmbu = player.level().getGameRules().getBoolean(OrbitalGameRules.ORBITAL_WEMMBU_ONLY);
        ...
        source.sendSuccess(() -> Component.literal(
                "Given orbital strike rod (" + type + ") targeting " +
                        x + " " + y + " " + z + " (value=" + value + ", delay=" + delay + ")"), true);

        return 1;
    }

    private static class OrbitalStrikeRodCommandSuggestions {
